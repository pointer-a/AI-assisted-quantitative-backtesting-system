...
CHECKPOINT_ROOT = Path(".exelixi") / "checkpoints"
...
def build_light_resume_inputs(runtime: Any, *, task: str | None = None, max_attempts: int = 3) -> dict[str, Any]:
    ...
    if not resume_task:
        resume_task = "Continue the interrupted Exelixi task from the checkpoint."
    else:
        resume_task = f"Continue this Exelixi task from the checkpoint: {resume_task}"
...
def normalize_resume_task(task: str) -> str:
    prefixes = (
        "Continue this Exelixi task from the checkpoint:",
        "Continue the interrupted Exelixi task from the checkpoint:",
    )
...
def snapshot_workspace_git(workspace: Path, root: Path) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        _git(workspace, git_dir, ["config", "user.name", "Exelixi Checkpoint"])
        _git(workspace, git_dir, ["config", "user.email", "Exelixi-checkpoint@example.local"])
...
def resume_command(workspace: Path) -> str:
    return f"uv run exelixi --resume {shlex.quote(str(workspace))}"
...
def should_skip_workspace_path(rel: Path) -> bool:
    parts = rel.parts
    if len(parts) >= 2 and parts[0] == ".exelixi" and parts[1] == "checkpoints":
        return True
