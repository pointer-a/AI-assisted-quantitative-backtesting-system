...
from exelixi.core.checkpoint import workspace_manifest


SESSION_ROOT = Path(".exelixi") / "session"
SESSION_FILE = "session.json"
SESSION_SUMMARY_FILE = "SESSION_SUMMARY.md"
MAX_RECENT_TURNS = 18
MAX_USER_TURN_CONTENT = 1800
MAX_ASSISTANT_TURN_CONTENT = 12000
MAX_WORKFLOW_EVENTS = 260
...
def append_user_turn(session: dict[str, Any], content: str) -> int:
    ...
    _append_turn(
        session,
        {
            ...
            "role": "user",
            "route": "",
            "content": trim_text(content, MAX_USER_TURN_CONTENT),
            "timestamp": utc_now(),
        },
    )
    return turn
...
def append_assistant_turn(
    ...
    *,
    ...
    session["last_final_answer"] = trim_text(content, MAX_ASSISTANT_TURN_CONTENT)
    _append_turn(
        session,
        {
            "turn": turn,
            "role": "assistant",
            "route": route,
            "content": trim_text(content, MAX_ASSISTANT_TURN_CONTENT),
            "summary": trim_text(summary or content, 700),
            "workflow_events": _trim_workflow_events(workflow_events or []),
            "timestamp": utc_now(),
        },
    )
...
def build_session_context(workspace: Path, session: dict[str, Any] | None = None) -> str:
    ...
    files = [
        ...
        if item.get("type") == "file" and not str(item.get("path", "")).startswith(".exelixi/session/")
    ][:30]
    recent_turns = [
        {
            "turn": turn.get("turn"),
...
def _trim_workflow_events(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    trimmed = []
    for event in events[-MAX_WORKFLOW_EVENTS:]:
        if not isinstance(event, dict):
            continue
        trimmed.append(
            {
                "kind": trim_text(str(event.get("kind", "")), 40),
                "type": trim_text(str(event.get("type", "")), 80),
                "node": trim_text(str(event.get("node", "")), 80),
                "name": trim_text(str(event.get("name", "")), 120),
                "title": trim_text(str(event.get("title", "")), 180),
                "detail": trim_text(str(event.get("detail", "")), 1600),
            }
        )
    return trimmed
