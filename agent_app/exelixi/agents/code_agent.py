...
from exelixi.graph.memory import build_layered_memory, format_layered_memory_for_prompt, memory_event
from exelixi.graph.state import ExelixiGraphState
from exelixi.prompts.stage3 import CODE_AGENT_PROMPT
from exelixi.providers.openai_provider import create_model, is_content_risk_error, safe_model_error
from exelixi.tools import build_tools
from exelixi.tools.todo_tool import persist_todos, update_todo
    ...
    *,
    ...
    for _ in range(max_loops):
        try:
            ...
            response = AIMessage(
                content=(
                    "codeAgent 的模型调用触发内容风险保护，已停止继续生成，并保留当前已完成的工具结果。"
                )
            )
...
def _code_agent_input(state: ExelixiGraphState, instruction: str, memory: dict[str, Any]) -> str:
    parts = [
        f"Task: {state['task']}",
