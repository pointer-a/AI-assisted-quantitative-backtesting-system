...
from exelixi.core.approval import ApprovalDecision, classify_command_risk, make_approval_request
...
def _normalize_command(command: str) -> str:
    if os.name == "nt":
        ...
        normalized = re.sub(
            r"^\s*cd\s+(?:/workspace|workspace|\.?/workspace|\.exelixi[\\/]+workspace)\s*(?:&&|&)\s*",
            "",
            normalized,
            count=1,
            flags=re.IGNORECASE,
        ...
        return normalized
    return re.sub(
        r"^\s*cd\s+(?:/workspace|workspace|\.?/workspace|\.exelixi[\\/]+workspace)\s*(?:&&|;)\s*pwd\s*$",
        "cd",
        command,
        count=1,
        flags=re.IGNORECASE,
...
def _handle_tail_command(state: RuntimeState, command: str) -> dict[str, Any] | None:
    ...
    from exelixi.tools.file_tools import read_text_lossy, resolve_workspace_path
...
def _build_env(state: RuntimeState) -> tuple[dict[str, str], str | None]:
    ...
    env_file = state.bash_env_file or state.workspace / ".exelixi.env"
    if env_file.exists():
        try:
...
def _ensure_toolchain_shims(state: RuntimeState) -> Path:
    shim_dir = state.workspace / ".exelixi" / "shims"
