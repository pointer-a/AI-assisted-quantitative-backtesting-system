...
from exelixi.tools.human_tool import request_write_approval
...
def write_file(state: RuntimeState, file_path: str, content: str) -> dict[str, Any]:
    ...
    action = "Modify file" if existed else "Create file"
    approval = request_write_approval(
        state,
        tool_name="FileWriteTool",
        action=action,
        path=display_path(state, path),
        preview=diff or f"{action}: {display_path(state, path)}\nLines: {len(content.splitlines())}",
    )
    if approval is not None and approval.get("ok") is False:
        return approval
