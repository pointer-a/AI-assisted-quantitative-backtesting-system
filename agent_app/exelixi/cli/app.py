...
from exelixi.cli.formatter import print_event, safe_echo, safe_secho
from exelixi.core.approval import ApprovalDecision, ApprovalRequest
from exelixi.core.agent import stream_agent_events


class ExelixiGroup(TyperGroup):
    """Let ``exelixi "task"`` coexist with real subcommands."""
...
app = typer.Typer(
    cls=ExelixiGroup,
    help='Exelixi: a teaching-first mini CodeAgent. Use `exelixi "task"` for Rich output or `exelixi tui` for Textual TUI.',
)
...
def main(
    ...
    workspace: Annotated[
        Path | None,
        typer.Option("--workspace", "-w", help="Workspace for generated files. Defaults to a fresh .exelixi/workspaces/workspace-* directory."),
    ] = None,
    ...
    resume: Annotated[
        Path | None,
        typer.Option("--resume", help="Resume from an existing Exelixi workspace."),
    ] = None,
) -> None:
    ...
    safe_secho("Exelixi stage 5: MultiAgent + context/harness engineering", fg=typer.colors.MAGENTA)
...
def tui(
    ...
    configure_console()
    from exelixi.cli.tui import ExelixiTuiApp
...
def _inline_approval_handler(request: ApprovalRequest) -> ApprovalDecision:
    from exelixi.cli.formatter import console
