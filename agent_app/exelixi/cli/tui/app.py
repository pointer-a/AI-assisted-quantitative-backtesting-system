...
from exelixi.cli.event_summary import EventSummary, shorten, summarize_event
from exelixi.cli.tui.approval import ApprovalGate, ApprovalModal
from exelixi.cli.tui.logo import render_logo
...
class ExelixiTuiApp(App[None]):
    ...
    def start_task(self, task: str, resume: Path | None = None) -> None:
        if self.running:
            self.notify("Exelixi is already running a task.", severity="warning")
            return
        self.running = True
        self.run_count += 1
        self.todos = []
        ...
        self.run_worker(lambda: self._run_stream(task, resume), thread=True, exclusive=False, name=f"Exelixi-run-{self.run_count}")
